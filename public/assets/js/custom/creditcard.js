$(document).ready(function () {
    if (window.location.pathname === "/credit-card/create") {
        console.log("script is running on create page");
        var currentTab = 0;
        showTab(currentTab);
        ...
        function validateForm() {
            ...
            if (currentTab == 0) {
                if (name === "") {
                    ...
                    $("#annual_income_error").text(
                        "Please enter your net annual income."
                    );
                    isValid = false;
                } else if (isNaN(income) || parseFloat(income) <= 0) {
                    $("#annual_income_error").text(
                        "Please enter a valid net annual income."
                    );
                    isValid = false;
                }
            } else if (currentTab == 1) {
                if ($("#residence_address").val().trim() === "") {
                    $("#residence_address_error").text(
                        "Please enter your address."
                    );
                    isValid = false;
                } else if ($("#office_address").val().trim() === "") {
                    $("#office_address_error").text(
                    ...
                    isValid = false;
                } else if (panFile.size > 2048 * 1024) {
                    // Max size in bytes (2048 KB)
                    $("#pan_file_error").text(
                        "File size should not exceed 2MB."
                    );
                    isValid = false;
                } else if (!adharFrontFile) {
                    $("#adhar_front_file_error").text(
                        "Please upload your Aadhar front."
                    );
                    isValid = false;
                    ...
                    isValid = false;
                } else if (adharFrontFile.size > 2048 * 1024) {
                    // Max size in bytes (2048 KB)
                    $("#adhar_front_file_error").text(
                        "File size should not exceed 2MB."
                    );
                    ...
                    $("#adhar_back_file_error").text(
                        "Please upload your Aadhar back."
                    );
                    isValid = false;
                    ...
                    isValid = false;
                } else if (adharBackFile.size > 2048 * 1024) {
                    // Max size in bytes (2048 KB)
                    $("#adhar_back_file_error").text(
                        "File size should not exceed 2MB."
                    );
                    ...
                    isValid = false;
                } else if (itrFile.size > 2048 * 1024) {
                    // Max size in bytes (2048 KB)
                    $("#itr_file_error").text(
                        "File size should not exceed 2MB."
                    );
                    ...
                    $("#bank_statement_file_error").text(
                        "Please upload your bank statement file."
                    );
                    isValid = false;
                } else if (
                    !allowedImageExtensions.test(bankStatementFile.name)
                ) {
                    $("#bank_statement_file_error").text(
                        "Please upload a PDF, JPEG, JPG, or PNG image."
                    );
                    isValid = false;
                } else if (bankStatementFile.size > 2048 * 1024) {
                    // Max size in bytes (2048 KB)
                    $("#bank_statement_file_error").text(
                        "File size should not exceed 2MB."
                    );
        ...
        $("#creditForm").submit(function (event) {
            ...
            if (validateForm()) {
                ...
                formData.append(
                    "adhar_front_file",
                    $("#adhar_front_file").prop("files")[0]
                );
                formData.append(
                    "adhar_back_file",
                    $("#adhar_back_file").prop("files")[0]
                );
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == true) {
                            ...
                            swal({
                                title: "Success!",
                                text: "Credit card application submitted successfully.",
                                icon: "success",
                                button: "OK",
