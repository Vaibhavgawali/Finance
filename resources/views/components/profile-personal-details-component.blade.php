    ...
    <div class="professional-birth-section card">
        <div class="d-flex justify-content-between p-3">
            ...
            @if (auth()->user()->id == $data->id)
                <button class="btn btn-gradient-primary btn-sm " id="professional_details_edit_button"><i
        ...
        <div class="professional-birth-div p-3">
            <form class="forms-sample" id="professional_details_update_form">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control p-4" id="profession" name="profession" placeholder="Profession"
                        disabled>
                        <option value="" selected disabled>--Select--</option>
                        <option value="Business"
                            {{ isset($data->profession) && $data->profession->profession === 'Business' ? 'selected' : '' }}>
                            Business</option>
                        <option value="Salary"
                            {{ isset($data->profession) && $data->profession->profession === 'Salary' ? 'selected' : '' }}>
                            Salary</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control p-4" name="company_name" id="company_name"
                        placeholder="Company Name" disabled value='{{ $data->profession->company_name ?? '' }}'>
                    <div id="company_name_error"></div>
                </div>

                <div class="form-group">
                    <label for="monthly_income">Monthly Income</label>
                    <select class="form-control p-4" id="monthly_income" name="monthly_income"
                        placeholder="Monthly Income" disabled>
                        <option value="" selected disabled>--Select--</option>
                        <option value="1"
                            {{ isset($data->profession) && $data->profession->monthly_income === '1' ? 'selected' : '' }}>
                            10k to 25k</option>
                        <option value="2"
                            {{ isset($data->profession) && $data->profession->monthly_income === '2' ? 'selected' : '' }}>
                            25k to 50k</option>
                        <option value="3"
                            {{ isset($data->profession) && $data->profession->monthly_income === '3' ? 'selected' : '' }}>
                            50k to 100k</option>
                        <option value="4"
                            {{ isset($data->profession) && $data->profession->monthly_income === '4' ? 'selected' : '' }}>
                            100k to 300k</option>
                        <option value="5"
                            {{ isset($data->profession) && $data->profession->monthly_income === '5' ? 'selected' : '' }}>
                            Above 300k</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control p-4" id="domain" name="domain" placeholder="Domain" disabled>
                        ...
                        <option value="Insurance"
                            {{ isset($data->profession) && $data->profession->domain === 'Insurance' ? 'selected' : '' }}>
                            Insurance</option>
                        <option value="Loan"
                            {{ isset($data->profession) && $data->profession->domain === 'Loan' ? 'selected' : '' }}>
                            Loan</option>
                        <option value="Sales"
                            {{ isset($data->profession) && $data->profession->domain === 'Sales' ? 'selected' : '' }}>
                            Sales</option>
                        <option value="Marketing"
                            {{ isset($data->profession) && $data->profession->domain === 'Marketing' ? 'selected' : '' }}>
                            Marketing</option>
                        <option value="Telecom"
                            {{ isset($data->profession) && $data->profession->domain === 'Telecom' ? 'selected' : '' }}>
                            Telecom</option>
                        <option value="Other"
                            {{ isset($data->profession->domain) && !in_array($data->profession->domain, ['Insurance', 'Loan', 'Sales', 'Marketing', 'Telecom']) ? 'selected' : '' }}>
                            Other</option>
                    </select>
                    <div id="domain_error"></div>
                </div>

                <div class="form-group" id="other_domain_field"
                    style="display: {{ !in_array($data->profession->domain ?? '', ['Insurance', 'Loan', 'Sales', 'Marketing', 'Telecom']) ? 'block' : 'none' }}">
                    <label for="other_domain">Enter Domain</label>
                    <input type="text" class="form-control" name="other_domain" id="other_domain"
                        placeholder="Other Domain" value='{{ $data->profession->domain ?? '' }}'>
                ...
                @if (auth()->user()->id == $data->id)
                    <div id="professional_details_update_button_div">
                        <button type="submit" class="btn btn-gradient-primary me-2"
                            id="professional_details_update_button">Update</button>
