...
class InsuranceController_old extends Controller
{
    ...
    {
        $this->middleware('auth:sanctum')->except('create', 'store', 'insuranceCallback', 'policyStatus');
    ...
    public function insuranceCallback(Request $request)
    {
        $validatedData = $request->validate([
            'InsurerID' => 'required|string|max:255',
            'InsurerName' => 'required|string|max:255',
            'EnquiryType' => 'required|string|max:255',
            'TotalPremium' => 'required|string|max:255',
            'EnquiryNo' => 'required|string|max:255',
            'BusinessID' => 'required|string|max:255',
            'PolicyNo' => 'required|string|max:255',
            'PolicyStatus' => 'required|string|max:255',
    ...
    public function policyStatus(Request $request)
    {
        ...
        $policyStatus = $request->query('policystatus');
        $policyNumber = $request->query('policynumber');
        $totalPremium = $request->query('totalpremium');
        $enquiryNo = $request->query('enquieryno');
        ...
        $validatedData = $request->validate([
            ...
            'totalpremium' => 'required|string|max:255',
            'enquieryno' => 'required|string|max:255',
        ]);

        return view('frontend.payment_success', compact('insurerId', 'policyStatus', 'policyNumber', 'totalPremium', 'enquiryNo'));
    }
}
